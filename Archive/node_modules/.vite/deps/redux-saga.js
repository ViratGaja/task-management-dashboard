...
var string3 = function string4(pattern) {
  return function(input) {
    return input.type === String(pattern);
  };
};
var symbol3 = function symbol4(pattern) {
  return function(input) {
...
function matcher(pattern) {
  var matcherCreator = pattern === "*" ? wildcard : string(pattern) ? string3 : array(pattern) ? array2 : stringableFunc(pattern) ? string3 : func(pattern) ? predicate : symbol(pattern) ? symbol3 : null;
...
function runTakeEffect(env, _ref3, cb) {
  var _ref3$channel = _ref3.channel, channel4 = _ref3$channel === void 0 ? env.channel : _ref3$channel, pattern = _ref3.pattern, maybe = _ref3.maybe;
  ...
  try {
    channel4.take(takeCb, notUndef(pattern) ? matcher(pattern) : null);
  } catch (err) {
    cb(err, true);
    return;
  }
...
function runChannelEffect(env, _ref12, cb) {
  var pattern = _ref12.pattern, buffer3 = _ref12.buffer;
  var chan = channel3(buffer3);
...
var toString = function toString2() {
  var firstSaga = sagaStack[0], otherSagas = _arrayLikeToArray(sagaStack).slice(1);
...
function sagaMiddlewareFactory(_temp) {
  var _ref = _temp === void 0 ? {} : _temp, _ref$context = _ref.context, context = _ref$context === void 0 ? {} : _ref$context, _ref$channel = _ref.channel, channel4 = _ref$channel === void 0 ? stdChannel() : _ref$channel, sagaMonitor = _ref.sagaMonitor, options = _objectWithoutPropertiesLoose(_ref, _excluded);
  var boundRunSaga;
  {
